...
# def find_nearest_hospitals(user_location, hospital_data, k=3):
#     def haversine(lat1, lon1, lat2, lon2):
#         # Haversine formula (same as in the previous code)

#     # Calculate distances from the user's location to hospitals
#     distances = [haversine(user_location[0], user_location[1], hospital[0], hospital[1]) for hospital in hospital_data]

#     # Use KNN to find the nearest hospitals
#     knn = NearestNeighbors(n_neighbors=k, metric='euclidean')
#     knn.fit(hospital_data)
#     nearest_hospitals = knn.kneighbors([user_location], return_distance=False)

#     nearest_hospital_info = []
#     for hospital_idx in nearest_hospitals[0]:
#         hospital = hospital_data[hospital_idx]
#         nearest_hospital_info.append({
#             'latitude': hospital[0],
#             'longitude': hospital[1],
#             'distance': distances[hospital_idx]
#         })

#     return nearest_hospital_info

# # Example usage:
# user_location = np.array([38.8951, -77.0369]) 
# nearest_hospitals = find_nearest_hospitals(user_location, hospital_data, k=3)

# # Use nearest_hospitals in your project as needed
